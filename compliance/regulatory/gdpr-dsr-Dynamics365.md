...
Le guide explique comment utiliser les outils d’administration, les services et les produits de Microsoft pour aider nos clients contrôleurs à rechercher et à agir sur des données personnelles pour répondre à des demandes DSR. Plus précisément, il décrit comment rechercher, accéder et agir sur des données personnelles ou des informations personnelles stockées dans le cloud de Microsoft. Voici un bref aperçu des processus décrits dans ce guide :
...
- Recherchez des données client à l’aide des fonctionnalités mentionnées dans « Rechercher des données client », puis modifiez directement les données dans les formulaires Customer Engagement. Les modifications peuvent être effectuées au niveau d’une ligne unique ou plusieurs lignes peuvent être modifiées directement.
...
En outre, pour Dynamics 365 Marketing, la suppression d’un contact garantira que les données d’interaction avec les informations personnelles seront supprimées également. Pour les entités ou champs personnalisés, vous devez personnaliser votre système afin de vous assurer qu’il supprime toutes les données client des enregistrements associés et/ou les dissocie de l’enregistrement de contact afin que toutes les données personnelles soient supprimées. Pour plus d’informations, voir : [Guide du développeur (Marketing)](/dynamics365/customer-engagement/marketing/developer/marketing-developer-guide).
...
- Vous pouvez supprimer des données client stockées dans un champ en supprimant manuellement l’élément de données contenant les données client cibles. Pour plus d’informations, consultez la section [Saisie de données](/dynamics365/business-central/ui-enter-data).
...
|**Quelles données l’outil d’exportation des journaux de données renvoie-t-il ?**| L’outil d’exportation des journaux de données renvoie des journaux générés par le système et stockés par Microsoft. Les données exportées englobent les différents services Microsoft, y compris Office 365, Azure et Dynamics. |
